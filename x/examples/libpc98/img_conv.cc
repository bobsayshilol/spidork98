...
int main_convert(int argc, const char ** argv) {
  if (argc != 6 && argc != 8) {
    printf(
      ...
      "    2 - use a default palette (<pal> should be 16, 32, or 64)\n"
      "    4 - create a sprite mask (<pal> is ignored)\n"
  ...
  images::Palette palette;
  if (flags & 4) {
    palette.num_colours = 2;
    palette.rgb[0] = palette.rgb[1] = palette.rgb[2] = 0;
    palette.rgb[3] = palette.rgb[4] = palette.rgb[5] = 0xFF;
  } else if (flags & 2) {
    const int palette_depth = atoi(palette_path);
    switch (palette_depth) {
      case 16: palette = images::default_palette_16; break;
      case 64: palette = images::default_palette_64; break;
      default:
        printf("Unsupported colour depth: %i\n", palette_depth);
        return EXIT_FAILURE;
    }
